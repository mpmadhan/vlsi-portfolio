...
### parity_generator_nbit
Takes N-bit data and generates an even parity bit.
If the number of 1s in data is already even, parity = 0.
If odd, parity = 1 to make the total count even.

### parity_checker_nbit
Takes N-bit data and the received parity bit, outputs an error flag.
If XOR of all (N+1) bits = 0 → no error.
If XOR of all (N+1) bits = 1 → error detected.
...
| Port | Direction | Width | Description |
|------|-----------|-------|-------------|
| d | input | N-bit | Data input |
| parity | output | 1-bit | Generated parity bit |

### parity_checker_nbit
| Port | Direction | Width | Description |
|------|-----------|-------|-------------|
| d | input | N-bit | Received data |
| parity | input | 1-bit | Received parity bit |
| error | output | 1-bit | 1 = error detected, 0 = no error |
...
| File | Description |
|------|-------------|
| parity_generator_4bit.v | Generates even parity bit for 4-bit data |
| parity_generator_4bit_tb.v | Testbench covering all 16 data combinations |
| parity_checker_4bit.v | Detects single bit errors using parity |
| parity_checker_4bit_tb.v | Testbench covering all 32 combinations (16 data × 2 parity) |
| parity_generator_nbit.v | Generates even parity bit for N-bit data |
| parity_generator_nbit_tb.v | Testbench covering all N^2(N sqaure) data combinations |
| parity_checker_nbit.v | Detects single bit errors using parity |
| parity_checker_nbit_tb.v | Testbench covering all combinations (N^2 data × 2 parity) |
...
### parity_checker_nbit
```verilog
//Testbench for N-Bit Parity Checker
`timescale 1ns/1ps
module parity_checker_nbit_tb();
  //1. Signal Declaration
  localparam N = 4;
  reg [(N-1):0] d;
  reg parity;
  ...
  integer i,j;
  //2. Dut Instantiation
  parity_checker_nbit #(.N(N)) dut(.d(d),.parity(parity),.error(error));
  //3. Waveform + Stimulus
  initial begin
    //3.1 Waveform
    $dumpfile("parity_checker_nbit_tb.vcd");
    $dumpvars(0,parity_checker_nbit_tb);
    //3.2 Display
    $display("Time |  D  Parity | Error");
    $display("-----|------------|------");
    //3.3 Stimulus
...
### parity_generator_4bit & parity_generator_nbit
